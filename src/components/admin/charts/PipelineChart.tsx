    ...
    <div className="space-y-2.5">
      {stages.map((s) => (
        <div key={s.label} className="flex items-center gap-3">
          <span className="w-32 shrink-0 truncate text-xs text-white/60">{s.label}</span>
          ...
          <span className="w-6 shrink-0 text-right text-sm font-semibold text-white/80">{s.value}</span>
        </div>
      ))}
